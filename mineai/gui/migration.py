...
class MigrationWindow(ctk.CTkToplevel):
    def __init__(self, parent, mc_dir: str, lang_label: str, cache_std, cache_ai, log_callback):
        ...
        ctk.CTkLabel(self, text="Выберите ресурс-пак с переводом (.zip):", font=("", 12, "bold")).pack(pady=(15, 5), padx=20, anchor="w")
        
        self.ent_zip = ctk.CTkEntry(self, width=300)
        self.ent_zip.pack(side="top", fill="x", padx=20, pady=5)
        
        ctk.CTkButton(self, text="Обзор", command=self._browse).pack(pady=5, padx=20, anchor="e")

        ctk.CTkLabel(self, text="Для какого движка использовать этот архив?", font=("", 12, "bold")).pack(pady=(10, 5), padx=20, anchor="w")
        self.var_cache = ctk.StringVar(value="ai")
        ctk.CTkRadioButton(self, text="Для Нейросетей (папка imported_caches/ai)", variable=self.var_cache, value="ai").pack(anchor="w", padx=20, pady=2)
        ctk.CTkRadioButton(self, text="Для Google/DeepL (папка imported_caches/std)", variable=self.var_cache, value="std").pack(anchor="w", padx=20, pady=2)

        self.btn_run = ctk.CTkButton(self, text="Начать миграцию", fg_color="#28a745", hover_color="#218838", command=self._run)
    ...
    def _run(self):
        ...
        def task():
            try:
                count = run_migration(zip_path, self.mc_dir, cache_type, self.lang_api_code, self.log_callback)
                if count > 0:
                    if cache_type == "ai":
